        ...
        if (username === ADMIN_USERNAME) {
            try {
                ...
                if (match) {
                    ...
                    const user: User = { id: 1, username: ADMIN_USERNAME }; // Hardcoded user object
...
passport.deserializeUser(async (id: number, done) => {
    ...
    if (id === 1) { // Our hardcoded admin user has id 1
        const user: User = { id: 1, username: ADMIN_USERNAME };
        done(null, user);
    } else {
...
app.get('/api/gallery/images', (req: Request, res: Response) => {
...
app.delete('/api/gallery/images/:id', isAuthenticated, (req: Request, res: Response) => {
    ...
    db.get(findSql, [imageId], (err: Error | null, row: { filepath: string } | undefined) => {
        if (err) {
